...
public class IgniteClusterMgrImpl implements IgniteClusterMgr {
    ...
    public void init() {
        ...
        logger.info("Ignite init with local address: " + localAddr + ", zkConnectionStr: "
                + CaimiContants.ZOOKEEPER_CLUSTER);
        // Ignite Server Mode：True
        // String igniteClientMode =
        // System.getProperty(ConfigConstants.SYSPROP_IGNITE_CLIENT_MODE);
        ...
        if (ignite == null) {
            ...
            logger.info("Ignite cluster started: " + cluster + ", cache names: " + ignite.cacheNames()
                    + ", local node: " + localNode.id() + ", order: " + localNode.version() + ", host names: "
                    + localNode.hostNames());
            recreateDmap();
